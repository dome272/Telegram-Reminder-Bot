...
def json_editor(user, key, value):
    ...
    with open("reminder.json", "r+") as file:
        ...
        if key == "name":
            content["reminder"][user].insert(0, {})
        content["reminder"][user][0][key] = value
        file.seek(0)
...
def json_getter(user, ):
    with open("reminder.json") as file:
        content = json.load(file)
        element = content["reminder"][user][0]
        name = element["name"]
        date = element["date"]
        _time = element["time"]
        r_id = element["id"]
...
def json_deleter(user, r_id=None, current=False):
    with open("reminder.json", "r+") as file:
        content = json.load(file)
        reminder = content["reminder"][user]
        if not current:
...
def all_reminder(update, context):
    ...
    with open("reminder.json") as file:
        content = json.load(file)
        reminder = content["reminder"][username]
        if len(reminder) == 0:
            update.message.reply_text("You don't have any reminders saved!")
        else:
...
def info(update, context):
    ...
    if text == "Yes":
        ...
        context.bot.send_message(chat_id=chat_id, text=f"*\U0001F4CC Saved Reminder *\U0001F4CC\n\nAppointment: {name}\nDate: {date}\nTime: {hour}:{minute} {m}", parse_mode="markdown", reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True, resize_keyboard=True))
